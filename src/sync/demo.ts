...
export async function demoStore(): Promise<Store> {
  ...
  const sofBd = addDays(t, 11).slice(5)
  s.put('config', { id: 'users', a: { name: 'Дмитрий', color: '#4b9cf5', g: 'm', birthday: `${y - 29}-03-02` }, b: { name: 'Софья', color: '#ea7266', g: 'f', birthday: `${y - 27}-${sofBd}` } } as never)
  ...
  const jp = s.put('plans', { title: 'Япония, октябрь', kind: 'trip', mode: 'goal', budget: 5000, currency: 'EUR', by: 'a' })
  s.put('expenses', { planId: jp.id, amount: 1200, kind: 'save', paidBy: 'a', note: 'С зарплаты', date: day(-60), by: 'a' })
  s.put('expenses', { planId: jp.id, amount: 700, kind: 'save', paidBy: 'b', note: 'С премии', date: day(-30), by: 'b' })
  s.put('expenses', { planId: jp.id, amount: 500, kind: 'save', paidBy: 'b', date: day(-5), by: 'b' })
  s.put('expenses', { planId: jp.id, amount: 640, kind: 'spend', paidBy: 'a', note: 'Билеты', date: day(-20), by: 'a' })
  const k = s.put('plans', { title: 'Кухня', kind: 'repair', mode: 'budget', budget: 180000, currency: 'RUB', by: 'a' })
  ...
  const v1 = s.put('votes', { question: 'Куда летим на майские?', options: ['Грузия', 'Армения', 'Турция'], by: 'b' })
  s.put('ballots', { id: `${v1.id}:b`, voteId: v1.id, user: 'b', choice: 0, comment: 'Хочу хинкали', by: 'b' })
  const v2 = s.put('votes', { question: 'Заводим кота?', options: ['Да', 'Нет'], by: 'a', createdAt: Date.now() - 5 * 86400000 })
  s.put('ballots', { id: `${v2.id}:a`, voteId: v2.id, user: 'a', choice: 0, by: 'a' })
  s.put('ballots', { id: `${v2.id}:b`, voteId: v2.id, user: 'b', choice: 0, comment: 'Рыжего!', by: 'b' })
  const v3 = s.put('votes', { question: 'Насколько хороша идея ремонта в ванной этим летом?', options: ['1', '2', '3', '4', '5'], by: 'a', createdAt: Date.now() - 2 * 86400000 })
  s.put('ballots', { id: `${v3.id}:a`, voteId: v3.id, user: 'a', choice: 3, by: 'a' })
